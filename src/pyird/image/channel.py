...
def channel_cube_to_image(channel_cube,revert=True):
    ...
    if revert:
        image=(revert_channel_cube(channel_cube).reshape(Nch*ch_pix_num, ysize)) # recover
    else:
...
def bias_region(channel_cube,margin=4):
    ...
    ref=np.concatenate([channel_cube[:,:,0:margin],channel_cube[:,:,ysize-margin:ysize]],axis=2)
